...
const EngagementTasks = () => {
  ...
  const [taskStatus, setTaskStatus] = useState<Record<string, boolean>>(
    JSON.parse(localStorage.getItem('engagementTaskStatus') || '{}')
  );
  ...
  useEffect(() => {
    ...
    localStorage.setItem('engagementTaskStatus', JSON.stringify(taskStatus));
  }, [taskStatus]);
  ...
  const handleTaskCompletion = async (task: string) => {
    setTaskStatus((prevStatus) => {
      ...
      localStorage.setItem('engagementTaskStatus', JSON.stringify(newStatus));
      return newStatus;
    });
  };
  ...
  return (
    <div className="p-6 border rounded-lg shadow-lg bg-gray-100 max-w-lg mx-auto">
      ...
      <h2 className="text-2xl font-bold mb-4 text-center text-gray-800">Engagement Tasks</h2>
